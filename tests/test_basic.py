...
class TestBasic(unittest.TestCase):
    ...
    @patch('os.path.getsize')
    @patch('sys.stdin')
    def test_init_overwrite_h_shortcut(self, mock_stdin, mock_getsize, mock_isfile, mock_open, mock_makedirs):
    ...
    @patch('os.path.getsize')
    @patch('sys.stdin')
    def test_init_overwrite_invalid_input(self, mock_stdin, mock_getsize, mock_isfile, mock_open, mock_makedirs):
        """Test that typing an invalid choice in the --init overwrite prompt warns and re-prompts."""
        mock_isfile.return_value = True
        mock_getsize.return_value = 100
        mock_stdin.isatty.return_value = True
        ...
        captured_output = io.StringIO()
        sys.stdout = captured_output
        with patch('builtins.input', side_effect=['invalid_option', 'n']) as mock_input:
            main()
            output = captured_output.getvalue()
            self.assertEqual(mock_input.call_count, 2)
            self.assertIn("Error:", output)
            self.assertIn("Invalid option 'invalid_option'. Please enter y, n, or help.", output)
            self.assertIn("Initialization aborted.", output)
            mock_open.assert_not_called()
    ...
    @patch('builtins.open')
    @patch('sys.stdin')
    def test_interactive_onboarding_h_shortcut(self, mock_stdin, mock_open, mock_makedirs, mock_isdir, mock_get_lib_version):
    ...
    @patch('builtins.open')
    @patch('sys.stdin')
    def test_interactive_onboarding_invalid_input(self, mock_stdin, mock_open, mock_makedirs, mock_isdir, mock_get_lib_version):
        """Test that typing an invalid choice in the onboarding prompt warns and re-prompts."""
        mock_get_lib_version.return_value = "1.2.3"
        mock_isdir.return_value = False
        mock_stdin.isatty.return_value = True
        ...
        with patch('builtins.input', side_effect=['maybe', 'n']) as mock_input:
            ...
            self.assertEqual(mock_input.call_count, 2)
            self.assertIn("Error:", output)
            self.assertIn("Invalid option 'maybe'. Please enter y, n, or help.", output)
            self.assertIn("Onboarding declined.", output)
