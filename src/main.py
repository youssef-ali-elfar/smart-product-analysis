...
def main():
    ...
    if data_dir_exists and data_count > 0:
        ...
        type_summary = ", ".join([f"{count} {ext}" for ext, count in sorted(type_counts.items())])

        names = ", ".join(files[:2])
        if data_count > 2:
            names += ", ..."
        freshness = f" - Updated {get_relative_time(freshest_time)}"
        data_status = f"✅ {GREEN}Found ({data_count} {suffix} [{type_summary}]: {names}, {size_str}){RESET}{freshness}"
    ...
    for i, stage in enumerate(stages, 1):
        ...
        # Colorize label and add indicator for the active step
        label_color = RESET
        current_indicator = ""
        ...
        print(f"{BOLD}{i}.{RESET} {stage['emoji']} {status_tag} {BOLD}{label_color}{stage['label']:<20}{RESET}: {stage['desc']}{current_indicator}")
