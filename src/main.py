...
def main():
    ...
    while True:
                ...
                if os.path.isfile(csv_path) and os.path.getsize(csv_path) > 0:
                    try:
                        ...
                        if lines:
                            ...
                            missing_cols = [col for col in headers if col in missing_col_set]
        ...
        if not all_found:
            ...
            cols_joined = natural_join([f"{BOLD}{c}{RESET}" for c in missing_cols]) if missing_cols else "your dataset"
            tip_text = f"Detected {BOLD}{missing_values_count} missing values{RESET} in {cols_joined} in your dataset. Proceed to {BOLD}Stage 2: Data Cleaning{RESET} to handle them!"
        elif not is_virtual:
