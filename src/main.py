...
def main():
    ...
    while True:
        ...
        if not all_found:
            ...
            formatted_cols = natural_join([f"{BOLD}{col}{RESET}" for col in missing_cols]) if missing_cols else "your dataset"
            tip_text = f"Detected {BOLD}{missing_values_count} missing values{RESET} in {formatted_cols} in your dataset. Proceed to {BOLD}Stage 2: Data Cleaning{RESET} to handle them!"
        elif not is_virtual:
