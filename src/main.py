...
def main():
    ...
    while True:
                ...
                if os.path.isfile(csv_path) and os.path.getsize(csv_path) > 0:
                    try:
                        ...
                        if lines:
                            ...
                            missing_cols = [col for col in headers if col in missing_cols_set]

                            warning_suffix = ""
                            if missing_values_count > 0:
                                col_list_str = ", ".join(missing_cols)
                                if args.plain:
                                    warning_suffix = f" ({missing_values_count} missing values in {col_list_str})"
                                else:
                                    warning_suffix = f" ({EMOJI_WARN}{missing_values_count} missing values in {col_list_str})"
        ...
        if not all_found:
            ...
            missing_joined = natural_join(missing_cols)
            tip_text = f"Detected {BOLD}{missing_values_count} missing values{RESET} in your dataset. Proceed to {BOLD}Stage 2: Data Cleaning{RESET} to handle them! (Missing in {missing_joined})"
        elif not is_virtual:
