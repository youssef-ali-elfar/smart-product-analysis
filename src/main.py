...
def main():
    ...
    libs = {
        ...
        "Seaborn": "seaborn", "Scikit-Learn": "sklearn", "Jupyter": "jupyter"
    }
    lib_statuses = {label: get_lib_version(name) for label, name in libs.items()}
    missing_libs = [l for l, v in lib_statuses.items() if v is None]
    ...
    if not all_found:
        badge, badge_color = "[INC]", "\033[91m"
    elif not data_dir_exists or data_count == 0:
        badge, badge_color = "[PEND]", "\033[93m"
    else:
        badge, badge_color = "[READY]", "\033[92m"
    ...
    if badge == "[READY]":
        badge_styled = f"{BOLD}{GREEN}{badge}{RESET}"
    elif badge == "[PEND]":
        badge_styled = f"{BOLD}{YELLOW}{badge}{RESET}"
    else:
        badge_styled = f"{BOLD}{RED}{badge}{RESET}"

    badge_padding = 14 - len(badge) - len(version)
    print(f"{BLUE}┌────────────────────────────────────────┐{RESET}")
    print(f"{BLUE}│ {BOLD}Smart Product Analysis{RESET} v{version}{' ' * badge_padding}{badge_styled} {BLUE}│{RESET}")
    print(f"{BLUE}└────────────────────────────────────────┘{RESET}")
    ...
    for label, lib_version in lib_statuses.items():
        if lib_version:
