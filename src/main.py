...
def main():
    ...
    while True:
                    ...
                    try:
                        ...
                        if lines:
                            ...
                            if missing_values_count > 0:
                                ...
                                if args.plain:
                                    warning_suffix = f" ({missing_values_count} missing values in {cols_str})"
                                else:
                                    warning_suffix = f" ({EMOJI_WARN}{missing_values_count} missing values in {cols_str})"
        ...
        if not all_found:
            ...
            missing_cols_formatted = natural_join([f"'{c}'" for c in missing_cols]) if args.plain else natural_join([f"{BOLD}{c}{RESET}" for c in missing_cols])
            tip_text = f"Detected {BOLD}{missing_values_count} missing values{RESET} in your dataset. Proceed to {BOLD}Stage 2: Data Cleaning{RESET} to handle them! (Missing in {missing_cols_formatted})"
        elif not is_virtual:
