...
def main():
    ...
    if data_dir_exists:
        ...
        for f in files:
            ...
            if mtime > freshest_time:
                freshest_time = mtime
                latest_file = f
                latest_file_size = f_size
            ext = os.path.splitext(f)[1][1:].upper() or "OTHER"
            file_types.append(ext)

        if latest_file:
            latest_file_with_size = f"{BOLD}{latest_file}{RESET} ({format_size(latest_file_size)})"
    ...
    if data_dir_exists and data_count > 0:
        ...
        print(f"• {'Data Source':<15}: ✅ {BOLD}{GREEN}Found in {DATA_DIR}/{RESET} ({data_count} {suffix}, {size_str}){integrity_warning}")
        if 1 <= data_count <= 3:
            file_details = []
            for f in files:
                f_size = os.path.getsize(os.path.join(DATA_DIR, f))
                file_details.append(f"{BOLD}{f}{RESET} ({format_size(f_size)})")
            file_list = natural_join(file_details)
            print(f"  - {'Files':<13}: {file_list}")
        elif data_count > 3:
            print(f"  - {'Latest':<13}: {latest_file_with_size}")
        print(f"  - {'Composition':<13}: {type_summary}")
        print(f"  - {'Freshness':<13}: {freshness}")
    elif data_dir_exists:
        data_status = f"⚠️ {BOLD}{YELLOW}Empty in {DATA_DIR}/{RESET} (0 files)"
        print(f"• {'Data Source':<15}: {data_status}")
    else:
        data_status = f"❌ {BOLD}{RED}Not Found{RESET} (looking for {BOLD}{DATA_DIR}/{RESET})"
        print(f"• {'Data Source':<15}: {data_status}")
    ...
    for i, stage in enumerate(stages, 1):
        ...
        if i == 1:
            if data_count > 0 and total_size > 0:
                ...
                status_tag = f"{CYAN}{BOLD}[NEXT]{RESET}"
                stage_color = CYAN
                is_current = True
            else:
                status_tag = f"{YELLOW}{BOLD}[PEND]{RESET}"
        ...
        current_indicator = f" {CYAN}{BOLD}◀ current{RESET}" if is_current else ""
        print(f"{BOLD}{i}.{RESET} {stage['emoji']} {status_tag} {stage_color}{BOLD}{stage['label']:<20}:{RESET} {stage['desc']}{current_indicator}")
    ...
    if not all_found:
        ...
        tip_text = f"Data files found in {BOLD}{DATA_DIR}/{RESET} appear to be empty (0 bytes). Please ensure your datasets contain valid product data."
    elif not is_virtual:
