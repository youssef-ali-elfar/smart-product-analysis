...
def main():
    ...
    while True:
                    ...
                    try:
                        ...
                        if lines:
                            ...
                            if missing_values_count > 0:
                                missing_suffix = "missing value" if missing_values_count == 1 else "missing values"
                                cols_str = f" in {', '.join(missing_cols)}" if missing_cols else ""
                                if args.plain:
                                    warning_suffix = f" ({missing_values_count} {missing_suffix}{cols_str})"
                                else:
                                    warning_suffix = f" ({EMOJI_WARN}{missing_values_count} {missing_suffix}{cols_str})"
        ...
        if not all_found:
            ...
            cols_text = f" in {cols_joined}" if cols_joined else ""
            tip_text = f"Detected {BOLD}{missing_values_count} {missing_suffix}{RESET}{cols_text} in your dataset. Proceed to {BOLD}Stage 2: Data Cleaning{RESET} to handle them!"
        elif not is_virtual:
