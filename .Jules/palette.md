...
## 2026-08-12 - Actionable Column-Specific Missing Values Context
**Learning:** Simply displaying a generic count of missing values in dataset checks provides incomplete guidance, forcing users to manually inspect or run extra diagnostics to locate the issues. Supplementing the warning count with the exact, sorted list of columns containing missing values in both the dataset status preview and the actionable cleaning tip dramatically reduces diagnostic overhead and guides users directly to their corrective actions.
**Action:** Track column-specific missing fields during CSV parsing and append a list of affected headers in natural language to dataset status previews and data-cleaning guidance tips.
